...
import { DeleteOutlined, EditOutlined, InboxOutlined, UserOutlined } from '@ant-design/icons'
import { Avatar, Button, Card, Form, Input, List, Modal, Popconfirm, Select, Tabs, Upload, message } from 'antd'
...
export const CourseDetailPage = () => {
  ...
  const handleRemoveParticipant = (participantName) => {
    const updatedParticipants = course.participants.filter((p) => p !== participantName)
  ...
  const renderMaterialsTab = () => {
    return (
      <div style={{ padding: 16 }}>
        {user.role === 'teacher' && (
          <Button type="primary" onClick={openMaterialModal} style={{ marginBottom: 16 }}>
            Загрузить материал
          </Button>
        )}
        ...
        <List
          ...
          renderItem={(material) => {
            ...
            if (user.role === 'teacher') {
              actions.push(
                <Button type="link" icon={<EditOutlined />} onClick={() => openEditMaterialModal(material)} />,
              )
              actions.push(
                <Popconfirm title="Удалить материал?" onConfirm={() => handleDeleteMaterial(material.id)}>
            ...
            return (
              <List.Item>
                <Card title={material.title} actions={actions} className="card-silka">
                  ...
                  {material.fileData && (
                    <Button onClick={() => openPdfInNewTab(material.fileData)}>Посмотреть документ</Button>
                  )}
                </Card>
              </List.Item>
            )
  ...
  const renderSubmissionsTab = () => {
    const visibleSubmissions =
      user.role === 'teacher'
        ? course.submissions
        : (course.submissions || []).filter((sub) => sub.studentId === user.username)
    ...
    return (
      <div style={{ padding: 16 }}>
        {user.role === 'student' && (
          <Button type="primary" onClick={openSubmissionModal} style={{ marginBottom: 16 }}>
            Загрузить работу
          </Button>
        )}
            ...
            return (
              <List.Item>
                <Card className="card-silka">
                  ...
                  <p>{sub.comment}</p>
                  <p>
                    <Button onClick={() => openPdfInNewTab(sub.fileData)}>Посмотреть документ</Button>
                  </p>
                  {sub.grade ? (
                    <p style={{ color: 'green' }}>
                  ...
                  {user.role === 'student' && isAuthor && (
                    <p style={{ fontSize: 12, color: '#999' }}>(Вы загрузили эту работу)</p>
                  )}
                </Card>
              </List.Item>
            )
  ...
  return (
    <div className="coursePage">
      ...
      {/* ============================= */}
      <Modal title="Редактировать курс" open={isEditModalOpen} onCancel={closeEditModal} onOk={handleEditCourse}>
      ...
      >
        <Form layout="vertical" form={materialForm}>
          <Form.Item label="Название материала" name="title" rules={[{ required: true, message: 'Введите название' }]}>
            <Input />
          </Form.Item>
      ...
      {/* ============================= */}
      <Modal title="Поставить оценку" open={isGradeModalOpen} onCancel={closeGradeModal} onOk={handleSetGrade}>
      ...
      >
        <Form layout="vertical" form={reflectionForm}>
          <Form.Item label="Ваш ответ" name="reflection" rules={[{ required: true, message: 'Введите ответ' }]}>
